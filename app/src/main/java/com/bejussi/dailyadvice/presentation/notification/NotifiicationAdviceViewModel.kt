...
@HiltViewModel
class NotifiicationAdviceViewModel @Inject constructor(
    ...
    fun getAdviceById(id: Int) {
        viewModelScope.launch {
            adviceRepository.getAdviceById(id).collect { result ->
                when (result) {
                    is Resource.Error -> {
                        ...
                        _eventFlow.send(
                            UIEvent.ShowToast(
                                result.message
                                    ?: stringResourcesProvider.getString(R.string.unknownError)
                            )
                        )
                    }
